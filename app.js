...
function renderSettings() {
  ...
  document.querySelector("#contactInfo").innerHTML = `구매 과정에서 문의 사항이 있으신 경우 전화 또는 문자로 문의해 주시기 바랍니다.<br>${contact}`;
...
async function renderMyOrders() {
  ...
  list.innerHTML = orders.map(order => {
    ...
    const returnAction = request
      ? `<div class="return-status">${escapeHtml(request.type)} ${escapeHtml(request.status)}<small>${escapeHtml(request.sellerMemo || "판매자가 요청을 확인하고 있습니다.")}</small></div>`
      : order.status !== "취소"
        ? `<button class="return-request-button" data-return-order="${order.id}">교환·환불 신청</button>`
        : "";
    return `<article class="my-order-card">
      ...
      ${returnAction ? `<div class="my-order-actions"><span>상품에 문제가 있나요?</span>${returnAction}</div>` : ""}
    </article>`;
  }).join("");
}
...
document.addEventListener("click", event => {
  ...
  if (returnOrder) {
    const order = customerOrdersCache.find(item => item.id === returnOrder.dataset.returnOrder);
    if (!order) return;
    const form = document.querySelector("#returnRequestForm");
    form.reset();
    form.elements.orderId.value = order.id;
    document.querySelector("#returnOrderNumber").textContent = `주문번호 ${order.id}`;
...
document.querySelector("#returnRequestClose").addEventListener("click", () => {
  document.querySelector("#returnRequestDialog").close();
  document.querySelector("#orderHistoryDialog").showModal();
});
document.querySelector("#returnRequestForm").addEventListener("submit", async event => {
  event.preventDefault();
  if (!event.currentTarget.reportValidity()) return;
  const formData = new FormData(event.currentTarget);
  const orderId = formData.get("orderId");
  const button = event.currentTarget.querySelector(".return-submit-button");
  button.disabled = true;
  button.textContent = "신청 접수 중…";
  try {
    const remote = await window.sansokFirebase.load();
    const orders = remote.orders || JSON.parse(localStorage.getItem("sansok-orders") || "[]");
    const order = orders.find(item => item.id === orderId && myOrderIds.includes(item.id));
    if (!order) throw new Error("주문을 찾을 수 없습니다.");
    if (order.returnRequest) throw new Error("이미 접수된 요청이 있습니다.");
    order.returnRequest = {
    ...
    document.querySelector("#orderHistoryDialog").showModal();
    showToast(`${order.returnRequest.type} 신청이 접수되었습니다.`);
  } catch (error) {
    console.error(error);
    showToast(error.message || "신청 접수에 실패했습니다.");
  } finally {
    button.disabled = false;
    button.textContent = "신청 접수하기";
  }
});
